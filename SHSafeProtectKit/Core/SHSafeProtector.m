...
static  SHSafeProtectorLogType sh_safe_logType=SHSafeProtectorLogTypeAll;
static  SHSafeProtectorBlock shSafeProtectorBlock;

@interface NSObject (SHSafeProtector)
//打开当前类安全保护
...
@implementation NSObject (SHSafeProtector)
@end

#pragma clang diagnostic pop


@implementation SHSafeProtector
...
+(void)openSafeProtectorWithIsDebug:(BOOL)isDebug block:(SHSafeProtectorBlock)block
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (isDebug) {
            sh_safe_logType=SHSafeProtectorLogTypeAll;
        }else{
            sh_safe_logType=SHSafeProtectorLogTypeNone;
        }
        shSafeProtectorBlock=block;
    });
}
...
+ (void)safe_logCrashWithException:(NSException *)exception crashType:(SHSafeProtectorCrashType)crashType
{
    ...
    if (shSafeProtectorBlock) {
        shSafeProtectorBlock(newException,crashType);
    }
    SHSafeProtectorLogType logType=sh_safe_logType;
    if (logType==SHSafeProtectorLogTypeNone) {
    }
    else if (logType==SHSafeProtectorLogTypeAll) {
        SHSafeLog(@"%@", fullMessage);
